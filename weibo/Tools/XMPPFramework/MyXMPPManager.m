...
- (BOOL)connect {
    ...
    //登陆用户的JID
    NSString *jidString = [NSString stringWithFormat:@"%@@%@",self.username,kYuMing];
...
- (void)registerUser:(NSString *)username password:(NSString *)password
    ...
    //匿名连接
    //匿名ID
    XMPPJID *JID = [XMPPJID jidWithString:[NSString stringWithFormat:@"anonymous@%@",kYuMing]];
    //设置连接的用户ID
    [_xmppStream setMyJID:JID];
    //连接
...
- (void)addFreind:(NSString *)freindUser {
    ...
    NSString *jid = [NSString stringWithFormat:@"%@@%@",freindUser,kYuMing];
    ...
    if ([iq.type isEqualToString:@"result"]) {
        ...
        if ([query.name isEqualToString:@"query"]) {
            ...
            for (DDXMLElement *item in [query children]) {
                ...
                NSString *ym = kYuMing;
                jid = [jid substringToIndex:jid.length - ym.length - 1];
                [data setObject:jid forKey:@"friendId"];
                [friends addObject:data];
            }
            
            //回调block，将好友列表传出
            if (self.freindBlcok != nil) {
                self.freindBlcok(friends);
            }
